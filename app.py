...
# returns a dict with the values of each answer
...
def get_value_of_answer(points_dict, answer):
    return points_dict.get(answer, 0)

points_dict = get_points_value(answers)
...
def index():
    ...
    if question:
        if request.method == 'POST':
            ...
            if submitted_answer in answers and submitted_answer not in correct_answers:
                correct_answers.append(submitted_answer)
               
                with open(answerlist, 'w') as json_file:
                    json.dump(correct_answers, json_file)
                
                # Print the value of the submitted answer to the console
